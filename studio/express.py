...
def _build_user_content(
    ...
    blocks.append({"type": "text", "text": (
        ...
        f"FIRST, CLASSIFY THE NEW REQUEST.  Two cases:\n\n"
        f"  CASE A — Targeted edit ('add X', 'change Y', 'remove Z', "
        f"  'highlight W', 'continue', 'explain step 3', 'colour the "
        f"  curve red').  The prior figure is essentially correct; "
        f"  the user wants a localised tweak.  Start from the prior "
        f"  SVG and apply ONLY the requested change.  Preserve every "
        f"  unchanged element BYTE-FOR-BYTE — same ids, coordinates, "
        ...
        f"  prior figure has a math error or layout failure.  DO NOT "
        f"  preserve it byte-for-byte — copying broken pixels keeps "
        f"  them broken.  Treat the prior figure ONLY as context to "
        f"  understand WHAT the user is pointing at; then DRAW A "
        f"  FRESH figure of the SAME CONCEPT, fixing the specific "
        f"  defect the user named.  Pick concrete numbers if the "
        f"  original prompt was vague.  Recompute every coordinate, "
        f"  every slope, every label position from scratch.  The "
        f"  goal is a CORRECT new figure, not a preserved-with-fix "
        f"  overlay of a broken old one.\n\n"
