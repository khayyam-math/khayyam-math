...
_EXTRACTOR_SYSTEM = (
    ...
    "  5b. Narration depth: emit a narration array with one phrase "
    "     PER LOGICAL STEP, not a single summary sentence.  Aim for "
    "     6–9 phrases for any Newton / step-by-step / iterative / "
    "     proof-walkthrough prompt (one phrase per iterate, plus an "
    "     intro and a conclusion); 4–6 for a single-concept figure "
    "     (one tangent, one intersection — intro, the construction, "
    "     a numerical check, the takeaway); 2–3 only for a trivial "
    "     'show the curve' request.  Lean LONG over SHORT — a "
    "     student who has time to listen is better served by an "
    "     extra phrase of intuition than by a terse list of values.  Each phrase one short sentence; "
    "     numbers spelled out for non-English narration.  Concrete "
    "     example for Newton on x³−2 from x_0 = 2: ['Newton's method "
    "     finds where a function crosses zero by following tangent "
    "     lines.', 'We start at x_0 = 2 where f equals six.', 'The "
    "     tangent at x_0 has slope f'(2) = 12, so it crosses the "
    "     x-axis at x_1 = 1.5.', 'At x_1 = 1.5, f is about 1.375; "
    "     the new tangent (slope 6.75) crosses at x_2 ≈ 1.296.', "
    "     'After two more iterations, the iterates converge to "
    "     x ≈ 1.26, the cube root of two.'].\n"
    ...
    "narration[] string in a non-English language, spell numbers out "
    "as words: German 1.5 → 'eineinhalb', Persian 3 → 'سه', French "
    "1.26 → 'un virgule deux six', Chinese 1.5 → '一点五'.  Captions "
    "(visible on the figure, not spoken) keep digits as numerals."
)
